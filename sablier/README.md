...
- **Distinction des terminaux par onglet** : les terminaux (Ghostty, iTerm2, Terminal, Alacritty, WezTerm, kitty) sont découpés par titre d'onglet, chaque onglet ayant sa fonction. Nécessite l'autorisation Accessibilité (les titres de fenêtres) et des onglets nommés de façon stable.
...
- **Assistant de catégorisation** : le rapport propose lui-même les gros postes non classés des 7 derniers jours, triés par temps (les broutilles sous 2 min sont ignorées). Un menu déroulant à côté de chaque poste range le site ou l'app dans une catégorie existante ou nouvelle, en un clic. La règle s'écrit toute seule en tête du fichier et tout l'historique se reclasse. Pas besoin de deviner : Sablier montre ce qui pèse, vous décidez.
